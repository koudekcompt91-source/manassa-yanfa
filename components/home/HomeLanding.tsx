...
import { teachers } from "@/lib/literature-data";
...
export default function HomeLanding() {
  ...
  const homeButtons = (ctaButtons || []).filter((row: any) => row.placement === "homepage" && row.visible);
  const featuredCourses = (packages || []).filter((row: any) => row.isFeatured).slice(0, 4);
  ...
  return (
    <div className="bg-slate-50 text-slate-900">
      <section
        ...
        className="border-b border-slate-800/40 bg-slate-950 bg-hero-mesh"
      >
          ...
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mx-auto mt-6 max-w-2xl text-pretty text-base leading-relaxed text-slate-300 sm:text-lg">
              {homepageContent?.heroSubtitle ||
                "منصة عربية متخصصة في النحو والبلاغة والشعر والنقد الأدبي: دروس مباشرة، قراءات موجهة، وتمارين تحليل نصوص بأسلوب أكاديمي واضح ومنظم، مع تحضير منهجي للاختبارات الأدبية."}
            </p>
            <div className="mt-10 flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:items-center sm:justify-center sm:gap-4">
              {homeButtons.slice(0, 3).map((btn: any, i: number) => (
                <Link
                  key={btn.id}
                  href={btn.route}
                  className={
                    i === 0
                      ? "inline-flex items-center justify-center rounded-2xl bg-white px-8 py-4 text-base font-bold text-slate-900 no-underline shadow-xl shadow-slate-950/30 transition hover:bg-slate-100"
                      : "inline-flex items-center justify-center rounded-2xl border border-white/25 bg-white/5 px-8 py-4 text-base font-semibold text-white no-underline backdrop-blur-sm transition hover:bg-white/10"
                  }
                >
                  {btn.label}
                </Link>
            ...
            <p className="mt-8 text-sm text-slate-400">يمكنك تجربة المحتوى الأدبي والتأكد من المنهج قبل الالتزام.</p>
          </div>
        </div>
      </section>

      {(announcements || [])
        .filter((row: any) => row.placement === "homepage" || row.placement === "global")
        .map((row: any) => (
          <div key={row.id} className={`${container} pt-4`}>
            <p className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">{row.title}</p>
          </div>
        ))}
      ...
      <section className="border-b border-slate-200/90 bg-white py-16 sm:py-20" aria-labelledby="features-title">
        <div className={container}>
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-3 text-slate-600 sm:text-lg">ثلاثة محاور نركّز عليها يوميًا مع كل دارس للأدب العربي.</p>
          </div>
          <ul className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            <li className="rounded-2xl border border-slate-200/90 bg-slate-50/80 p-6 shadow-card transition hover:border-brand-200 hover:shadow-card-hover">
              ...
              <h3 className="mt-5 text-lg font-bold text-slate-900">قراءات أدبية مباشرة</h3>
              <p className="mt-2 text-sm leading-relaxed text-slate-600 sm:text-base">
                جلسات تفاعلية لشرح النصوص والشواهد مع نقاش مباشر يرسّخ الفهم البلاغي واللغوي.
              </p>
            </li>
            <li className="rounded-2xl border border-slate-200/90 bg-slate-50/80 p-6 shadow-card transition hover:border-brand-200 hover:shadow-card-hover">
              ...
              <h3 className="mt-5 text-lg font-bold text-slate-900">متابعة تحليلية شخصية</h3>
              <p className="mt-2 text-sm leading-relaxed text-slate-600 sm:text-base">
                تغذية راجعة على أسلوبك في التحليل والكتابة النقدية، مع توجيه واضح لبناء منهجية رصينة.
              </p>
            </li>
            <li className="rounded-2xl border border-slate-200/90 bg-slate-50/80 p-6 shadow-card transition hover:border-brand-200 hover:shadow-card-hover sm:col-span-2 lg:col-span-1">
              ...
              <h3 className="mt-5 text-lg font-bold text-slate-900">إتقان أدبي تراكمي</h3>
              <p className="mt-2 text-sm leading-relaxed text-slate-600 sm:text-base">
                خطة تمارين ومراجعات نصية متدرجة لتقوية أدواتك في النحو والبلاغة والنقد الأدبي.
              </p>
            </li>
          </ul>
        </div>
      ...
      <section className="border-b border-slate-200/90 bg-slate-50 py-16 sm:py-20" aria-labelledby="teachers-title">
        <div className={container}>
          <div className="mx-auto max-w-2xl text-center">
            <h2 id="teachers-title" className="text-2xl font-extrabold text-slate-900 sm:text-3xl md:text-4xl">
              نخبة الأساتذة المتخصصين
            </h2>
            <p className="mt-3 text-slate-600 sm:text-lg">خبرات أكاديمية في فروع الأدب العربي القديمة والحديثة.</p>
          </div>
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {teachers.map((teacher) => (
              <article key={teacher.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
                <h3 className="text-lg font-bold text-slate-900">{teacher.name}</h3>
                <p className="mt-1 text-sm font-semibold text-brand-700">{teacher.specialization}</p>
      ...
      <section className="border-b border-slate-200/90 bg-white py-16 sm:py-20" aria-labelledby="tracks-title">
        <div className={container}>
          <div className="mx-auto max-w-2xl text-center">
            <h2 id="tracks-title" className="text-2xl font-extrabold text-slate-900 sm:text-3xl md:text-4xl">
              {homepageContent?.featuredHeading || "مسارات أكاديمية متخصصة"}
            </h2>
            <p className="mt-3 text-slate-600 sm:text-lg">
              برنامج منظم يشبه الأكاديميات الاحترافية: مسارات واضحة، أهداف تعلم دقيقة، وتقييمات دورية.
            </p>
          </div>
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {(featuredCourses.length ? featuredCourses.map((c: any) => c.title) : [
              "النحو العربي",
              "البلاغة",
              "الأدب الجاهلي",
              "الأدب الأموي",
              ...
              <div key={track} className="rounded-2xl border border-slate-200/90 bg-slate-50/70 px-4 py-3 text-sm font-bold text-slate-800 shadow-sm">
                {track}
              </div>
            ))}
          </div>
      ...
      <section className="border-b border-slate-200/90 bg-slate-50 py-16 sm:py-20" aria-labelledby="journey-title">
        <div className={container}>
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-3 text-slate-600 sm:text-lg">سجّل، التحق بمسار منظم، ثم تقدّم بثقة في التحصيل والاختبارات الأدبية.</p>
          </div>
          <ol className="mt-12 grid gap-8 md:grid-cols-3">
            {[
              { n: "١", t: "سجّل", d: "أنشئ حسابك واختر مسارك الأدبي المناسب: نحو، بلاغة، شعر، أو نقد." },
              { n: "٢", t: "حلّل", d: "احضر الجلسات، اقرأ النصوص، وطبّق منهجيات التحليل خطوة بخطوة." },
              { n: "٣", t: "أتقن", d: "طوّر كتابتك الأدبية والنقدية عبر تدريبات منتظمة وتوجيه مستمر." },
            ].map((step) => (
      ...
      <section className="border-b border-slate-800/50 bg-slate-950 py-14 sm:py-16" aria-label="أرقام المنصة">
        <div className={container}>
          <div className="grid gap-6 sm:grid-cols-3">
            {[
              { v: "+5000", l: "دارس أدب", s: "انضمّوا لمساراتنا الأدبية" },
              { v: "+95%", l: "رضا وتقييم إيجابي", s: "وفق استبيانات ما بعد الجلسات" },
              { v: "+200", l: "وحدة أدبية", s: "محتوى متجدد في فروع الأدب" },
            ].map((row) => (
              <div
                key={row.l}
      ...
      <section className="border-b border-slate-200/90 bg-white py-16 sm:py-20" aria-labelledby="reviews-title">
        <div className={container}>
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-3 text-slate-600 sm:text-lg">تجارب مختصرة من دارسي الأدب العربي في المنصة.</p>
          </div>
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {[
              {
                name: "ياسمين العلوي",
                role: "مسار البلاغة والنقد",
                q: "الجلسات المباشرة حسّنت قدرتي على تحليل الصور البيانية وربطها بسياق النص.",
              },
              {
                name: "أمين بنجلون",
                role: "مسار النحو العربي",
                q: "التدريبات التطبيقية بعد كل درس جعلت الإعراب أسهل وأكثر دقة في الكتابة.",
              },
              {
                name: "سارة الإدريسي",
                role: "مسار الشعر والعروض",
                q: "شرح الأوزان والتقطيع كان عمليًا جدًا، وأصبحت أقرأ القصيدة بوعي موسيقي أفضل.",
              },
            ].map((r) => (
            ...
            {[
              ...
              >
                ...
                <p className="mt-6">
                  ...
                  <span className="ms-1 text-sm font-medium text-slate-500">دج / شهر</span>
                </p>
      ...
      <section className="bg-white py-16 sm:py-20" aria-labelledby="cta-title">
        <div className={container}>
          <div className="rounded-3xl bg-gradient-to-l from-brand-700 via-indigo-800 to-slate-900 px-6 py-12 text-center shadow-xl sm:px-12 sm:py-16">
            <div className="relative z-10 mx-auto max-w-2xl">
              <h2 id="cta-title" className="text-2xl font-extrabold text-white sm:text-3xl md:text-4xl">
                {homepageContent?.ctaTitle || "جاهز لتوسيع أفقك الأدبي؟"}
              </h2>
              <p className="mt-4 text-slate-200 sm:text-lg">
                {homepageContent?.ctaSubtitle || "أنشئ حسابك وابدأ تجربة أدبية منظمة — نحن هنا لدعمك في النحو والبلاغة والشعر والنقد."}
              </p>
      ...
      <footer id="contact" className="border-t border-slate-200 bg-slate-100/90">
        <div className={`${container} py-12 sm:py-14`}>
          <div className="flex flex-col gap-10 md:flex-row md:items-start md:justify-between md:gap-8">
            <div className="max-w-md">
              ...
              <p className="mt-4 text-sm leading-relaxed text-slate-600">
                {homepageContent?.supportText || "أكاديمية عربية متخصصة في الأدب العربي: دروس مباشرة، تحليل نصوص، ومتابعة تبني منهجية رصينة."}
              </p>
          ...
          <div className="mt-10 flex flex-col items-center justify-between gap-4 border-t border-slate-200/80 pt-8 text-center text-xs text-slate-500 sm:flex-row sm:text-start">
            ...
            <p className="text-slate-400">{settings?.footerText || homepageContent?.footerText || "صُممت لخدمة الدرس الأدبي العربي."}</p>
          </div>
        </div>
      </footer>
    </div>
