...
/**
 * Student PAID فروضي — PaidFileContent ASSIGNMENT via /api/paid-content.
 * Access filtered server-side (Enrollment / accessType / level / PAID course).
 */
export default function TeachersAssignmentsPanel() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await fetch("/api/paid-content?contentType=ASSIGNMENT", {
        credentials: "include",
        cache: "no-store",
      });
      ...
      if (!res.ok || !data?.ok) {
        setItems([]);
        setError(data?.message || "تعذّر تحميل الفروض.");
        return;
      }
      setItems(Array.isArray(data.items) ? data.items : []);
    } catch {
      setItems([]);
      setError("حدث خطأ أثناء تحميل الفروض.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  if (loading) {
    return (
      <section className="rounded-2xl border border-slate-200 bg-white px-5 py-12 text-center shadow-sm">
        <p className="text-sm font-semibold text-slate-500">جاري تحميل الفروض…</p>
      </section>
    );
  }
  ...
  if (!items.length) {
    return (
      <section className="rounded-2xl border border-dashed border-slate-200 bg-white px-5 py-12 text-center shadow-sm">
        <p className="text-base font-extrabold text-slate-800">لا توجد فروض متاحة حاليًا.</p>
      </section>
    );
  }

  return (
    <section aria-label="فروضي" className="space-y-3">
      {items.map((item) => {
        ...
        return (
          ...
          >
            <div className="flex min-w-0 items-start gap-3">
              <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-600 to-indigo-700 text-white">
