...
/**
 * Student PAID diagnostics — PaidDiagnosticContent via /api/paid-diagnostics.
 * Opens resource with target=_blank + rel=noopener (same safe pattern as summaries).
 */
export default function TeachersDiagnosticPanel() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await fetch("/api/paid-diagnostics", {
        credentials: "include",
        cache: "no-store",
      });
      ...
      if (!res.ok || !data?.ok) {
        setItems([]);
        setError(data?.message || "تعذّر تحميل التقويات التشخيصية.");
        return;
      }
      setItems(Array.isArray(data.items) ? data.items : []);
    } catch {
      setItems([]);
      setError("حدث خطأ أثناء تحميل التقويات التشخيصية.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  if (loading) {
    return (
      <section className="rounded-2xl border border-slate-200 bg-white px-5 py-12 text-center shadow-sm">
        <p className="text-sm font-semibold text-slate-500">جاري تحميل التقويات…</p>
      </section>
    );
  }
