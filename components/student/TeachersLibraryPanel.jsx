...
/**
 * Student PAID مكتبتي — PaidFileContent LIBRARY via /api/paid-content.
 * Access filtered server-side (Enrollment / accessType / level / PAID course).
 */
export default function TeachersLibraryPanel() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await fetch("/api/paid-content?contentType=LIBRARY", {
        credentials: "include",
        cache: "no-store",
      });
      ...
      if (!res.ok || !data?.ok) {
        setItems([]);
        setError(data?.message || "تعذّر تحميل المكتبة.");
        return;
      }
      setItems(Array.isArray(data.items) ? data.items : []);
    } catch {
      setItems([]);
      setError("حدث خطأ أثناء تحميل المكتبة.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  if (loading) {
    return (
      <section className="rounded-2xl border border-slate-200 bg-white px-5 py-12 text-center shadow-sm">
        <p className="text-sm font-semibold text-slate-500">جاري تحميل المكتبة…</p>
      </section>
    );
  }
  ...
  if (!items.length) {
    return (
      <section className="rounded-2xl border border-dashed border-slate-200 bg-white px-5 py-12 text-center shadow-sm">
        <p className="text-base font-extrabold text-slate-800">لا توجد موارد متاحة حاليًا.</p>
      </section>
    );
  }

  return (
    <section aria-label="مكتبتي" className="space-y-3">
      {items.map((item) => {
        ...
        return (
          ...
          >
            <div className="flex min-w-0 items-start gap-3">
              <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-600 to-slate-800 text-white">
                <Library className="h-7 w-7" strokeWidth={1.75} />
              </span>
