...
import { ELECTRONIC_QUIZ_POINTS_PER_QUESTION, ELECTRONIC_QUIZ_QUESTION_COUNT } from "@/lib/assessments";
...
export async function PATCH(req: Request, { params }: { params: { submissionId: string } }) {
  ...
  try {
    ...
    const isQuiz = submission.assessment.type === "QUIZ";
    ...
    const updates = answers
      .map((item: any) => {
        ...
        const maxPoints = isQuiz ? ELECTRONIC_QUIZ_POINTS_PER_QUESTION : Number(existing.question.points || 0);
        ...
        return prisma.assessmentAnswer.update({
          where: { id: answerId },
          data: {
            pointsAwarded: pointsAwarded > maxPoints ? maxPoints : pointsAwarded,
            correctionNote,
    ...
    const maxScore = isQuiz
      ...
      : updatedAnswers.reduce((acc, row) => acc + (Number(row.question.points) || 0), 0);
    let score = scoreOverride !== undefined ? Math.max(0, Math.round(Number(scoreOverride) || 0)) : computedScore;
    if (isQuiz) score = Math.min(ELECTRONIC_QUIZ_QUESTION_COUNT, score);
