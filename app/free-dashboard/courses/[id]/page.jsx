    ...
    fetch(`/api/free/courses/${encodeURIComponent(courseId)}`, { credentials: "include" })
      ...
      .then((data) => {
        ...
        if (!data?.ok || !data.course) {
          setError(data?.message || "الدورة غير متاحة.");
          setCourse(null);
          return;
        }
        setCourse(data.course);
        try {
          setDone(localStorage.getItem(progressKey(data.course.id)) === "1");
        } catch {
          setDone(false);
        }
      })
    ...
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-8" dir="rtl">
      ...
      <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-sm sm:p-6">
        ...
        <p className="mt-3 text-sm leading-7 text-slate-600 sm:text-base">
          {(course.description || "").trim() || "درس مجاني ضمن نظام التعلم المجاني."}
        </p>
      </div>
