...
/**
 * PAID اختباراتي admin — PaidFileContent contentType=EXAM (PDF).
 * Distinct from interactive Assessment.QUIZ under «اختبار إلكتروني».
 */
export default function AdminPaidExamsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!itemsRes.ok || !itemsData?.ok) {
        setError(itemsData?.message || "تعذّر تحميل الاختبارات.");
        setItems([]);
      } else {
  ...
  async function save(e) {
    ...
    try {
      ...
      const res = await fetch(isEdit ? `/api/admin/paid-content/${editingId}` : "/api/admin/paid-content", {
      ...
      if (!res.ok || !data?.ok) {
        setError(data?.message || "تعذّر حفظ الاختبار.");
        return;
      }
  ...
  async function removeRow(id) {
    ...
    const res = await fetch(`/api/admin/paid-content/${id}`, { method: "DELETE", credentials: "include" });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || !data?.ok) {
  ...
  async function togglePublish(row) {
    const res = await fetch(`/api/admin/paid-content/${row.id}`, {
      method: "PATCH",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <AdminShell
      title="اختباراتي — المحتوى المدفوع"
      subtitle="إدارة PaidFileContent بنوع EXAM ضمن دورات Course.system = PAID فقط."
    >
      <div className="mb-4">
        <Link
      ...
      <AdminSectionCard title="إضافة / تعديل اختبار" subtitle="PDF عبر رابط آمن · مرتبط بدورة مدفوعة.">
        ...
        <form className="mt-2 grid gap-3 md:grid-cols-2" onSubmit={save} dir="rtl">
          ...
          <AdminFormField label="عنوان الاختبار">
          ...
          <AdminFormField label="المادة">
            <AdminInput value={form.subject} onChange={(e) => setForm((s) => ({ ...s, subject: e.target.value }))} placeholder="مثال: علوم" />
          </AdminFormField>
          <AdminFormField label="رابط PDF للاختبار" className="md:col-span-2">
            <AdminInput
              value={form.fileUrl}
          ...
          <AdminFormField label="اسم الملف (اختياري)">
            <AdminInput value={form.fileName} onChange={(e) => setForm((s) => ({ ...s, fileName: e.target.value }))} />
          </AdminFormField>
          ...
          <div className="md:col-span-2 flex flex-wrap gap-2">
            <AdminActionButton type="submit" tone="primary" disabled={saving || !courses.length}>
              {saving ? "جاري الحفظ…" : editingId ? "حفظ التعديل" : "إضافة اختبار"}
            </AdminActionButton>
            {editingId ? (
      ...
      <AdminSectionCard title="اختبارات PAID" subtitle={`${items.length} اختبار · contentType = EXAM`}>
        {loading ? (
          <p className="text-sm text-slate-500">جاري التحميل…</p>
        ) : !items.length ? (
          <AdminEmptyState title="لا توجد اختبارات بعد" description="أضف أول اختبار من النموذج أعلاه." />
        ) : (
                ...
                {items.map((row) => (
                  <tr key={row.id} className="border-b border-slate-100">
                    <td className="px-3 py-3">
                      ...
                      {row.fileName ? <p className="mt-1 text-xs text-slate-500">{row.fileName}</p> : null}
                    </td>
