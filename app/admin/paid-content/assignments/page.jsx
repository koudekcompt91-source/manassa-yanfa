...
/**
 * PAID فروضي admin — PaidFileContent contentType=ASSIGNMENT (PDF).
 * Distinct from interactive Assessment.ASSIGNMENT under «واجباتي المنزلية».
 */
export default function AdminPaidAssignmentsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!itemsRes.ok || !itemsData?.ok) {
        setError(itemsData?.message || "تعذّر تحميل الفروض.");
        setItems([]);
      } else {
  ...
  async function save(e) {
    ...
    try {
      ...
      if (!res.ok || !data?.ok) {
        setError(data?.message || "تعذّر حفظ الفرض.");
        return;
      }
  ...
  async function removeRow(id) {
    if (!window.confirm("حذف هذا الفرض؟")) return;
  ...
  return (
    <AdminShell
      title="فروضي — المحتوى المدفوع"
      subtitle="إدارة PaidFileContent بنوع ASSIGNMENT ضمن دورات Course.system = PAID فقط."
    >
      <div className="mb-4">
        <Link
      ...
      <AdminSectionCard title="إضافة / تعديل فرض" subtitle="PDF عبر رابط آمن · مرتبط بدورة مدفوعة.">
        ...
        <form className="mt-2 grid gap-3 md:grid-cols-2" onSubmit={save} dir="rtl">
          ...
          <AdminFormField label="عنوان الفرض">
          ...
          <AdminFormField label="رابط PDF للفرض" className="md:col-span-2">
            <AdminInput
              value={form.fileUrl}
          ...
          <div className="md:col-span-2 flex flex-wrap gap-2">
            <AdminActionButton type="submit" tone="primary" disabled={saving || !courses.length}>
              {saving ? "جاري الحفظ…" : editingId ? "حفظ التعديل" : "إضافة فرض"}
            </AdminActionButton>
            {editingId ? (
      ...
      <AdminSectionCard title="فروض PAID" subtitle={`${items.length} فرض · contentType = ASSIGNMENT`}>
        {loading ? (
          <p className="text-sm text-slate-500">جاري التحميل…</p>
        ) : !items.length ? (
          <AdminEmptyState title="لا توجد فروض بعد" description="أضف أول فرض من النموذج أعلاه." />
        ) : (
