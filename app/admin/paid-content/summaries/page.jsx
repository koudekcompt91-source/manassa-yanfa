...
/**
 * PAID summaries admin — PaidFileContent contentType=SUMMARY only.
 */
export default function AdminPaidSummariesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!itemsRes.ok || !itemsData?.ok) {
        setError(itemsData?.message || "تعذّر تحميل الملخصات.");
        setItems([]);
      } else {
  ...
  async function save(e) {
    ...
    try {
      ...
      if (!res.ok || !data?.ok) {
        setError(data?.message || "تعذّر حفظ الملخص.");
        return;
      }
  ...
  async function removeRow(id) {
    if (!window.confirm("حذف هذا الملخص؟")) return;
  ...
  return (
    <AdminShell
      title="ملخصاتي — المحتوى المدفوع"
      subtitle="إدارة PaidFileContent بنوع SUMMARY ضمن دورات Course.system = PAID فقط."
    >
      <div className="mb-4">
        <Link
      ...
      <AdminSectionCard title="إضافة / تعديل ملخص" subtitle="PDF عبر رابط آمن · مرتبط بدورة مدفوعة.">
        ...
        <form className="mt-2 grid gap-3 md:grid-cols-2" onSubmit={save} dir="rtl">
          ...
          <AdminFormField label="عنوان الملخص">
          ...
          <AdminFormField label="المادة">
            <AdminInput value={form.subject} onChange={(e) => setForm((s) => ({ ...s, subject: e.target.value }))} placeholder="مثال: فيزياء" />
          </AdminFormField>
          <AdminFormField label="رابط PDF" className="md:col-span-2">
            <AdminInput
              value={form.fileUrl}
          ...
          <div className="md:col-span-2 flex flex-wrap gap-2">
            <AdminActionButton type="submit" tone="primary" disabled={saving || !courses.length}>
              {saving ? "جاري الحفظ…" : editingId ? "حفظ التعديل" : "إضافة ملخص"}
            </AdminActionButton>
            {editingId ? (
      ...
      <AdminSectionCard title="ملخصات PAID" subtitle={`${items.length} ملخص · contentType = SUMMARY`}>
        {loading ? (
          <p className="text-sm text-slate-500">جاري التحميل…</p>
        ) : !items.length ? (
          <AdminEmptyState title="لا توجد ملخصات بعد" description="أضف أول ملخص من النموذج أعلاه." />
        ) : (
