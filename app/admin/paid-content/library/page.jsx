...
/**
 * PAID مكتبتي admin — PaidFileContent contentType=LIBRARY (any file URL).
 */
export default function AdminPaidLibraryPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!itemsRes.ok || !itemsData?.ok) {
        setError(itemsData?.message || "تعذّر تحميل المكتبة.");
        setItems([]);
      } else {
  ...
  async function save(e) {
    ...
    try {
      ...
      if (!res.ok || !data?.ok) {
        setError(data?.message || "تعذّر حفظ المورد.");
        return;
      }
  ...
  async function removeRow(id) {
    if (!window.confirm("حذف هذا المورد من المكتبة؟")) return;
  ...
  return (
    <AdminShell
      title="مكتبتي — المحتوى المدفوع"
      subtitle="إدارة PaidFileContent بنوع LIBRARY ضمن دورات Course.system = PAID فقط."
    >
      <div className="mb-4">
        <Link
      ...
      <AdminSectionCard title="إضافة / تعديل مورد" subtitle="رابط ملف عام (PDF أو غيره) · مرتبط بدورة مدفوعة.">
        ...
        <form className="mt-2 grid gap-3 md:grid-cols-2" onSubmit={save} dir="rtl">
          ...
          <AdminFormField label="عنوان المورد">
          ...
          <AdminFormField label="المادة">
            <AdminInput value={form.subject} onChange={(e) => setForm((s) => ({ ...s, subject: e.target.value }))} placeholder="مثال: لغة عربية" />
          </AdminFormField>
          <AdminFormField label="رابط الملف" className="md:col-span-2">
            <AdminInput
              value={form.fileUrl}
          ...
          <div className="md:col-span-2 flex flex-wrap gap-2">
            <AdminActionButton type="submit" tone="primary" disabled={saving || !courses.length}>
              {saving ? "جاري الحفظ…" : editingId ? "حفظ التعديل" : "إضافة مورد"}
            </AdminActionButton>
            {editingId ? (
      ...
      <AdminSectionCard title="مكتبة PAID" subtitle={`${items.length} مورد · contentType = LIBRARY`}>
        {loading ? (
          <p className="text-sm text-slate-500">جاري التحميل…</p>
        ) : !items.length ? (
          <AdminEmptyState title="لا توجد موارد بعد" description="أضف أول مورد من النموذج أعلاه." />
        ) : (
