...
/**
 * Dedicated PAID student route for أساتذتي → دروسي.
 * Static path takes precedence over /dashboard/teachers/[section].
 */
export default function TeachersLessonsPage() {
  const section = getTeachersSection("lessons");
  const Icon = section?.Icon || BookOpen;
  const tone = section?.tone || "from-brand-600 to-indigo-600";
  const title = section?.title || "دروسي";
  const description =
    section?.description || "دروسك المسجّلة المرتبطة بأساتذتك ومسارك الدراسي المدفوع.";
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 sm:gap-8">
        ...
        <header className="rounded-2xl border border-slate-200/80 bg-white px-6 py-7 shadow-[0_10px_28px_-20px_rgba(15,23,42,0.18)] sm:px-8">
          <div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:items-start sm:text-right">
            <span
              className={`flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br text-white shadow-[0_10px_24px_-12px_rgba(37,99,235,0.4)] ${tone}`}
              aria-hidden
            >
