...
export default function TeachersElectronicExamPage() {
  const section = getTeachersSection("electronic-exam");
  const Icon = section.Icon;
  return (
    <div className="w-full" dir="rtl">
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 sm:gap-8">
        <Link href="/dashboard/teachers" className="inline-flex items-center gap-1.5 text-sm font-bold text-brand-700 hover:underline">
        ...
        <header className="rounded-2xl border border-slate-200/80 bg-white px-6 py-7 shadow-[0_10px_28px_-20px_rgba(15,23,42,0.18)] sm:px-8">
          <div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:items-start sm:text-right">
            <span className={`flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br text-white ${section.tone}`} aria-hidden>
        ...
        <TeachersAssessmentsPanel
          assessmentType="QUIZ"
          emptyTitle="لا توجد اختبارات إلكترونية متاحة حاليًا."
        />
      </div>
    </div>
  );
