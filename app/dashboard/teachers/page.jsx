...
const TEACHERS_SECTION_ID = "teachers-section";

function scrollToTeachersSection() {
  if (typeof window === "undefined") return;
  if (window.location.hash !== `#${TEACHERS_SECTION_ID}`) return;
  const el = document.getElementById(TEACHERS_SECTION_ID);
  if (!el) return;
  // Wait one frame so the section is laid out after navigation/render.
...
export default function TeachersHubPage() {
  useEffect(() => {
    // Brief delay so Next.js route scroll-to-top does not cancel the smooth scroll.
    const t = window.setTimeout(scrollToTeachersSection, 80);
    window.addEventListener("hashchange", scrollToTeachersSection);
    return () => {
      window.clearTimeout(t);
      window.removeEventListener("hashchange", scrollToTeachersSection);
    };
  }, []);
  ...
  return (
    <div className="w-full" dir="rtl">
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 sm:gap-8">
        ...
        {/* Target for nav card «أساتذتي» — scroll-mt clears sticky site header */}
        <div id={TEACHERS_SECTION_ID} className="scroll-mt-24 flex flex-col gap-6 sm:gap-8">
